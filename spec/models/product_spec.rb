...
RSpec.describe Product, type: :model do
  ...
  describe 'Validations' do
    ...
    it "it is not valid without a price" do
      @product = Product.create(name: "AirPods", quantity: 20, category: @category)
      expect(@product.errors.full_messages).to include("Price can't be blank")
    end
    it "it is not valid without a quantity" do
      @product = Product.create(name: "AirPods", price: 110, category: @category)
      expect(@product.errors.full_messages).to include("Quantity can't be blank")
    end
    it "it is not valid without a category" do
      @product = Product.create(name: "AirPods", price: 110, quantity: 20)
      expect(@product.errors.full_messages).to include("Category can't be blank")
    end
    it "should have the validation name, price, quantity and category present" do
      @product = Product.create(name: "AirPods", price: 110, quantity: 20, category: @category)
